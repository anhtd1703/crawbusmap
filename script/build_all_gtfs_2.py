...
def main():
    ...
    sf, shape_writer = prepare_gtfs_file(shapes_path, ["shape_id", "shape_pt_lat", "shape_pt_lon", "shape_pt_sequence"], region_code)
    tf, trip_writer = prepare_gtfs_file(trips_path, ["route_id", "service_id", "trip_id", "shape_id", "direction_id"], region_code)
    stf, stoptime_writer = prepare_gtfs_file(stoptimes_path, ["trip_id", "arrival_time", "departure_time", "stop_id", "stop_sequence"], region_code)
    ...
    for idx, detail_file in enumerate(detail_files):
        ...
        for d, stations in directions.items():
            ...
            for lat, lon in raw_points:
                shape_points.append({"lat": lat, "lon": lon})
            ...
            for st_idx, st in enumerate(stations):
                ...
                # Calculate travel time offset based on actual distance between consecutive stops
                if last_valid_lat is None:
                    current_time_offset = 0.0
                else:
                    dist_step = haversine(last_valid_lat, last_valid_lon, slat, slon)
                    current_time_offset += dist_step / SPEED_MS
                
                stop_offsets.append({
                    "stop_id": stop_id,
                    "sequence": len(stop_offsets) + 1,
                    "time_offset": int(current_time_offset)
                })
                last_valid_lat, last_valid_lon = slat, slon
            ...
            for t_idx, start_seconds in enumerate(timetables):
                ...
                for so in stop_offsets:
                    arr_dep_time = seconds_to_hhmmss(start_seconds + so["time_offset"])
